...
try {
    ...
    // NUEVO: Lógica para productos recomendados
    $productos_recomendados = [];
    if (isset($producto['categoriaID'])) {
        $stmt_recomendados = $pdo->prepare("
            SELECT
            ...
            LIMIT 8
        ");
        $stmt_recomendados->execute([$producto['categoriaID'], $id_producto_base]);
...
<body>
    ...
    <main class="page-container">
        ...
        <?php else: ?>
            ...
            <section class="ofertas-box" style="margin-top: 1rem;">
                <h2 class="ofertas-title">También te podría interesar</h2>
                <div class="carousel-container">
                    <button class="carousel-btn prev" aria-label="Anterior">&lt;</button>
                    <div class="carousel-viewport">
                        <div class="carousel-track">
                            <?php foreach ($productos_recomendados as $rec_producto): ?>
                                <?php
                                    $precio_final_rec = $rec_producto['precio'] * (1 - ($rec_producto['descuento'] ?? 0) / 100);
                                ?>
                                <div class="producto-box">
                                    <a href="productos.php?id=<?= $rec_producto['id_producto_base'] ?>">
                                        <img src="<?= htmlspecialchars($rec_producto['imagen_a_mostrar'] ?: 'Imagenes/placeholder.png') ?>" alt="<?= htmlspecialchars($rec_producto['nombre']) ?>" />
                                    </a>
                                    <h3><?= htmlspecialchars($rec_producto['nombre']) ?></h3>
                                    <p class="producto-precio">$<?= number_format($precio_final_rec, 0, ',', '.') ?></p>
                                    <button class="btn-add-to-cart" onclick="agregarAlCarrito({id: <?= $rec_producto['id_variante'] ?>, name: '<?= htmlspecialchars(addslashes($rec_producto['nombre'])) ?>', price: <?= $precio_final_rec ?>, image: '<?= htmlspecialchars(addslashes($rec_producto['imagen_a_mostrar'] ?: 'Imagenes/placeholder.png')) ?>', quantity: 1})">
                                      Agregar al carrito
                                    </button>
                                </div>
                            <?php endforeach; ?>
