...
try {
    // Consulta para el producto principal
    ...
    // 1. Obtener productos comprados frecuentemente junto a este
    $sql_juntos = "
        SELECT p2.*, MIN(v2.precio) as precio_desde
        ...
        WHERE v1.id_producto = ? AND p2.id != ? AND v2.stock > 0 AND p2.activo = 1
        GROUP BY p2.id
        ORDER BY COUNT(pi1.pedido_id) DESC
        LIMIT 4
    ";
    $stmt_juntos = $pdo->prepare($sql_juntos);
    $stmt_juntos->execute([$id_producto_base, $id_producto_base]);
    $productos_juntos = $stmt_juntos->fetchAll(PDO::FETCH_ASSOC);

    // 2. Obtener otros productos de la misma categoría
    $sql_categoria = "
        SELECT p.*, MIN(v.precio) as precio_desde
        FROM productos p
        LEFT JOIN variantes_producto v ON p.id = v.id_producto
        WHERE p.categoriaID = ? AND p.id != ? AND v.stock > 0 AND p.activo = 1
        GROUP BY p.id
        ORDER BY RAND()
        LIMIT 4
    ";
    $stmt_categoria = $pdo->prepare($sql_categoria);
    $stmt_categoria->execute([$producto['categoriaID'], $id_producto_base]);
    $productos_categoria = $stmt_categoria->fetchAll(PDO::FETCH_ASSOC);
...
<body>
    ...
    <main class="page-container">
        ...
        <?php else: ?>
                            ...
                            <div class="form-group">
                                ...
                                <select id="variante_select" class="form-control">
                                    <?php foreach ($variantes as $variante): ?>
                                        <option value="<?= $variante['id_variante'] ?>"><?= htmlspecialchars($variante['sku']) ?> - $<?= number_format($variante['precio'], 0, ',', '.') ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
            ...
            <?php if (!empty($productos_juntos) || !empty($productos_categoria)): ?>
            <section class="recommendation-section">
                <h2 class="recommendation-main-title">Recomendados para ti</h2>
                
                <?php if (!empty($productos_juntos)): ?>
                    <h3 class="recommendation-subtitle">Comprados Juntos Frecuentemente</h3>
                    <div class="recommendation-grid">
                        <?php foreach ($productos_juntos as $p_junto): ?>
                            <div class="producto-box">
                                <a href="productos.php?id=<?= $p_junto['id'] ?>">
                                    <img src="<?= htmlspecialchars($p_junto['imagen_principal'] ?: 'Imagenes/placeholder.png') ?>" alt="<?= htmlspecialchars($p_junto['nombre']) ?>">
                                </a>
                                <h3><?= htmlspecialchars($p_junto['nombre']) ?></h3>
                                <p class="producto-precio">Desde $<?= number_format($p_junto['precio_desde'], 0, ',', '.') ?></p>
                                <a href="productos.php?id=<?= $p_junto['id'] ?>" class="btn-add-to-cart">Ver Opciones</a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
                ...
                <?php if (!empty($productos_categoria)): ?>
                    <h3 class="recommendation-subtitle">Más en esta Categoría</h3>
                    <div class="recommendation-grid">
                         <?php foreach ($productos_categoria as $p_cat): ?>
                            <div class="producto-box">
                                <a href="productos.php?id=<?= $p_cat['id'] ?>">
                                    <img src="<?= htmlspecialchars($p_cat['imagen_principal'] ?: 'Imagenes/placeholder.png') ?>" alt="<?= htmlspecialchars($p_cat['nombre']) ?>">
                                </a>
                                <h3><?= htmlspecialchars($p_cat['nombre']) ?></h3>
                                <p class="producto-precio">Desde $<?= number_format($p_cat['precio_desde'], 0, ',', '.') ?></p>
                                <a href="productos.php?id=<?= $p_cat['id'] ?>" class="btn-add-to-cart">Ver Opciones</a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
        ...
        if(addToCartBtn) {
            addToCartBtn.addEventListener('click', () => {
                ...
                const productoParaCarrito = {
                    id: varianteSeleccionada.id_variante,
                    name: `<?= htmlspecialchars(addslashes($producto['nombre'])) ?> (${varianteSeleccionada.sku})`,
                    price: varianteSeleccionada.precio,
                    image: varianteSeleccionada.imagen || '<?= htmlspecialchars(addslashes($producto['imagen_principal'])) ?>',
